  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden animate-pulse">
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="relative w-full lg:w-72 xl:w-80 flex-shrink-0 aspect-[4/3] lg:aspect-auto lg:h-56 bg-slate-200 dark:bg-slate-700" />
...
export default function HotelBrowserModal({
  ...
  const filteredHotels = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'recommended':
        default:
          // Reddit score + rating combination
  ...
  return (
    <AnimatePresence>
      ...
      {/* Modal container - centered with proper sizing (not full screen) */}
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 20 }}
        transition={{ type: 'spring', damping: 25, stiffness: 300 }}
        className="fixed inset-0 sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 z-50 bg-slate-50 dark:bg-slate-900 rounded-none sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden sm:w-[90vw] md:w-[85vw] lg:w-[80vw] sm:max-w-6xl sm:h-[90vh] sm:max-h-[800px]"
      ...
      >
        ...
        <header className="sticky top-0 z-20 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm">
          <div className="max-w-7xl mx-auto">
            ...
            {/* Sort bar */}
            <div className={`flex items-center justify-between px-2 sm:px-4 py-2 sm:py-2.5 bg-slate-50 dark:bg-slate-800/50 overflow-x-auto ${isLoading ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="flex items-center gap-1.5 sm:gap-2 no-scrollbar">
                {/* Filter button */}
                <button
                ...
                <div className="flex items-center gap-1 sm:gap-1.5" role="group" aria-label="Sort options">
                  {SORT_OPTIONS.map(option => (
                    <button
                      ...
                      disabled={isLoading}
                      className={`min-h-[44px] px-2 sm:px-3 py-2 rounded-full text-xs sm:text-sm font-medium transition-colors whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-orange-500 disabled:cursor-not-allowed ${
                        sortBy === option.value
        ...
        {/* Info banner */}
        <div className="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700" role="note">
          <div className="max-w-7xl mx-auto px-4 py-2">
            <p className="text-xs text-slate-600 dark:text-slate-400 flex items-center gap-2">
              <AlertCircle className="w-3.5 h-3.5" aria-hidden="true" />
              Prices shown are per night. Actual prices may vary based on your dates and room selection.
            </p>
          </div>
        </div>
            ...
            */}
            <div className="space-y-3">
              {isLoading ? (
                ...
                <div className="text-center py-16 px-4 bg-white dark:bg-slate-800 rounded-xl">
                  ...
                  <button
                    ...
                    className="text-orange-600 hover:text-orange-700 font-medium focus:outline-none focus:ring-2 focus:ring-orange-500 rounded px-2 py-1"
                  >
                    Clear all filters
                  </button>
                </div>
                ...
                <>
                  ...
                  {visibleHotels.map((hotel, index) => (
                    <HotelListCard
                      ...
                      onViewDetails={() => setDetailHotel(hotel)}
                    />
                  ))}
                  ...
                  {hasMoreHotels && (
                    <div className="py-6 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-6 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
...
function HotelListCard({
  ...
  return (
    <article
      className={`bg-white dark:bg-slate-800 rounded-xl shadow-sm hover:shadow-md transition-all border border-slate-200 dark:border-slate-700 overflow-hidden ${
        isSelected ? 'ring-2 ring-orange-500 bg-orange-50/30 dark:bg-orange-900/10' : ''
      }`}
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row">
        {/* Image section */}
        <div className="relative w-full lg:w-72 xl:w-80 flex-shrink-0 aspect-[4/3] lg:aspect-auto lg:h-56 bg-slate-100 dark:bg-slate-700 overflow-hidden">
          <FallbackImage
            src={hotel.imageUrl}
          ...
          <button
            ...
            className="absolute inset-0 bg-black/0 hover:bg-black/20 transition-colors flex items-center justify-center opacity-0 hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-inset"
            aria-label={`View details for ${hotel.name}`}
          >
            <span className="bg-white/90 px-3 py-1.5 rounded-lg text-sm font-medium text-slate-700 shadow-md">
              View Details
            </span>
          </button>
        </div>
        ...
        <div className="flex-1 p-3 sm:p-4 lg:p-5 flex flex-col lg:flex-row">
          {/* Hotel details */}
          <div className="flex-1 min-w-0">
            <h3 className="text-base sm:text-lg font-semibold text-slate-900 dark:text-white mb-1 hover:text-orange-600 cursor-pointer truncate" onClick={onViewDetails}>
              {hotel.name}
            </h3>
          ...
          <div className="mt-3 lg:mt-0 lg:ml-6 lg:w-52 lg:flex-shrink-0 lg:border-l lg:border-slate-200 dark:lg:border-slate-700 lg:pl-6 flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-start gap-3 sm:gap-4 lg:gap-0 pt-3 sm:pt-0 border-t sm:border-t-0 lg:border-t-0 border-slate-100 dark:border-slate-700">
            ...
            <div className="flex-1 sm:flex-initial">
              ...
              {hasExceptionalValue && (
                <div className="mb-2 sm:mb-3">
                  <span className="inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] sm:text-xs font-medium rounded-full">
                    ✓ Great Value
                  </span>
                </div>
              )}
              ...
              {hotel.pricePerNight ? (
                <div className="space-y-0.5 sm:space-y-1">
                  <p className={`text-xl sm:text-2xl font-bold ${
                    hotel.priceConfidence === 'real' ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'
                  }`}>
                  ...
                  {totalPrice && nightCount > 1 && (
                    <div className="pt-1 sm:pt-2 mt-1 sm:mt-2 border-t border-slate-100 dark:border-slate-700 hidden sm:block">
                  ...
                  {hotel.priceConfidence === 'real' && (
                    <p className="text-[10px] sm:text-xs text-orange-600 flex items-center gap-1 mt-1 sm:mt-2">
                ...
                <p className="text-xs sm:text-sm text-slate-400 italic">Price on request</p>
              )}
            </div>
            ...
            <button
              ...
              className={`min-h-[44px] w-full sm:w-auto lg:w-full lg:mt-4 px-4 sm:px-6 py-2.5 rounded-xl font-medium text-xs sm:text-sm transition-all focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 ${
                isSelected
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Search */}
          <div>
            <label htmlFor="hotel-search" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
              Search Hotels
            </label>
            <input
              id="hotel-search"
              ...
              className="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-900 dark:text-white dark:bg-slate-700 placeholder-slate-400 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
          ...
          {/* Sort By */}
          <div>
            <label htmlFor="hotel-sort" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
              Sort By
            </label>
            <select
              id="hotel-sort"
              ...
              className="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-slate-900 dark:text-white dark:bg-slate-700 focus:ring-2 focus:ring-orange-500"
            >
              {SORT_OPTIONS.map(option => (
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-4 mb-3">
              <div>
                <span className="text-xs text-slate-500 mb-1 block">Daily Rate in Dollars</span>
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    <label htmlFor="price-min" className="text-xs text-slate-400">Min</label>
                    <div className="relative">
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true">$</span>
                      <input
                        id="price-min"
                        type="number"
                        value={filters.priceRange.min}
                        onChange={(e) => setFilters(f => ({ ...f, priceRange: { ...f.priceRange, min: Number(e.target.value) } }))}
                        className="w-full pl-6 pr-2 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500"
                  ...
                  <div className="flex-1">
                    <label htmlFor="price-max" className="text-xs text-slate-400">Max</label>
                    <div className="relative">
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true">$</span>
                      <input
                        id="price-max"
          ...
          <fieldset>
            ...
            <div className="space-y-2" role="group" aria-label="Filter by star rating">
              {[5, 4, 3].map(star => (
                <label key={star} className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-slate-700 dark:text-slate-300">{star} stars</span>
                </label>
              ))}
            </div>
          </fieldset>
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="Filter by guest rating">
              {[
                ...
                <label key={option.value} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 text-orange-500 border-slate-300 focus:ring-orange-500 accent-orange-500"
                  />
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-2" role="group" aria-label="Filter by amenities">
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-2 gap-2" role="group" aria-label="Filter by property type">
          ...
          <fieldset>
            ...
            <p className="text-xs text-slate-500 mb-3">Accessibility</p>
            <div className="grid grid-cols-2 gap-2" role="group" aria-label="Filter by accessibility features">
              {[
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between bg-white dark:bg-slate-800 safe-area-inset-bottom">
              <button
                onClick={onClearAll}
                className="min-h-[44px] px-3 sm:px-4 py-2.5 text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white focus:outline-none focus:ring-2 focus:ring-orange-500 rounded-lg"
              ...
              <button
                onClick={onClose}
                className="min-h-[44px] px-5 sm:px-6 py-2.5 text-xs sm:text-sm font-medium text-white bg-gradient-to-r from-orange-500 to-amber-500 rounded-xl hover:from-orange-600 hover:to-amber-600 shadow-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2"
              >
                Apply Filters
              </button>
            </div>
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center">
      ...
      <motion.div
        ...
        transition={{ type: 'spring', damping: 25, stiffness: 300 }}
        className="relative bg-white dark:bg-slate-800 w-full h-full sm:h-auto sm:max-w-4xl sm:max-h-[90vh] overflow-auto overscroll-contain rounded-none sm:rounded-2xl shadow-2xl"
        style={{ WebkitOverflowScrolling: 'touch' }}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="px-3 sm:px-6 pt-3 sm:pt-4 flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          {hotel.redditScore && hotel.redditScore > 2 && (
            <span className="inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 bg-orange-50 border border-orange-200 text-orange-700 text-[10px] sm:text-xs font-medium rounded-full">
              🔥 Reddit Favorite
            </span>
          )}
          {hotel.isAllInclusive && (
            <span className="inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 bg-purple-50 border border-purple-200 text-purple-700 text-[10px] sm:text-xs font-medium rounded-full">
              ✨ All-Inclusive
            </span>
          )}
        </div>
        ...
        <div className="px-3 sm:px-6 grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2" role="region" aria-label="Hotel photos">
          ...
          <div className="hidden sm:block aspect-[4/3] rounded-lg overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 relative" role="img" aria-label="Additional hotel photo">
            ...
            <button
              className="absolute bottom-2 right-2 bg-white dark:bg-slate-700 px-2 py-1 rounded text-[10px] sm:text-xs flex items-center gap-1 focus:outline-none focus:ring-2 focus:ring-orange-500 shadow-sm"
              aria-label="See all photos of this hotel"
            >
          ...
          <div className="px-3 sm:px-6 flex gap-4 sm:gap-6" role="tablist" aria-label="Hotel information tabs">
            {['Details', 'Amenities', 'Location'].map(tab => (
              <button
                ...
                className={`min-h-[44px] py-2 sm:py-3 text-xs sm:text-sm font-medium border-b-2 transition-colors focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-inset ${
        ...
        <div className="p-3 sm:p-6">
          ...
          {activeTab === 'location' && (
            <div role="tabpanel" id="tabpanel-location" aria-labelledby="tab-location">
              ...
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mb-3 sm:mb-4">{hotel.address || 'Address not available'}</p>
              {hotel.googleMapsUrl && (
                <a
                  href={hotel.googleMapsUrl}
                  target="_blank"
              ...
              {hotel.evidence
                ...
                .map((ev, i) => (
                  <div key={i} className="border-l-2 border-orange-300 pl-2 sm:pl-3">
                    ...
                    {ev.subreddit && (
                      <p className="text-[10px] sm:text-xs text-slate-400 mt-1">r/{ev.subreddit} · {ev.score} upvotes</p>
                    )}
                  </div>
                ))
              }
        ...
        <div className="sticky bottom-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-3 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 shadow-lg safe-area-inset-bottom">
          <div>
            {hotel.pricePerNight ? (
              <>
                ...
                {totalPrice && nightCount > 1 && (
                  <p className="text-xs sm:text-sm text-slate-500">
                    ${totalPrice.toLocaleString()} total for {nightCount} nights
                  </p>
                )}
              </>
            ) : (
              <p className="text-xs sm:text-sm text-slate-500">Price on request</p>
            )}
          </div>
          <button
            onClick={onSelect}
            className={`min-h-[48px] sm:min-h-[44px] w-full sm:w-auto px-6 sm:px-8 py-3 rounded-xl font-medium text-sm sm:text-base transition-all focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 ${
              isSelected
                ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/25'
                : 'bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white shadow-md hover:shadow-lg'
            }`}
